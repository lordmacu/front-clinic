...
<div class="d2" style="">
   ...
   <div class="bg" style="width:100vw; margin-top:-0px; position:absolute; background-size: cover; background-repeat: no-repeat; height:Calc(100vh - 170px); min-height:700px; z-index:0; top:0px; width:100%; background:linear-gradient(311deg, rgba(0,55,60,1) 0%, rgba(3,107,117,1) 100%);">
      -->
   <div class="bg" style="width:100vw; max-height:800px; margin-top:-0px; position:absolute; background-size: cover; background-repeat: no-repeat; height:Calc(100vh - 170px); min-height:750px; z-index:0; top:0px; width:100%; background: linear-gradient(311deg, rgba(2,92,122,1) 0%, rgba(2,134,178,1) 100%);">
      <div class="x1" style="position:relative; height:100%; max-width:1500px;  margin:0 auto;">
      <div class="persona-g">
      <div :style="{'background-image':'url(' + require('@/assets/personas/2.png') + ') '}" style="position:absolute; bottom:-1px; width: 100%; height:540px; background-position: right;  background-repeat: no-repeat;background-size:375px; " > 
      
      </div>
      <!--<img style="position:absolute; bottom:-1px; right:-330px; " src="@/assets/personas/2.png" alt="">-->
      <div style="position:absolute; color:white; bottom:30px; right:340px; font-weight:700; font-size:18px;"> Gabriela, enfermera certificada</div>
      </div>
      </div>
   </div>
   <div class="d1" style=" top:0px; left: 0; right: 0; margin-left: auto; margin-right: auto; max-width:800px; ">
      ...
      <div style="position:relative; margin-top:0vh ">
         ...
         <div class="h2container2" style="padding: 0px 0px;">
            ...
            <div class="h2container">
               ...
               <h5 class="subtitulo"><strong>Paga solo el copago</strong>: reembolsa de manera <strong>automática e inmediata</strong> cuando te tomemos el examen, con <strong>Isapre o Fonasa</strong></h5>
               <barra-busqueda :key="c" v-if="window.width >= 1000" />
               <barra-busqueda-movil v-if="window.width < 1000" />
<div  v-if="examenes_agregados.length > 0" class="container" 
    style=" margin-top:18px; width:800px; max-width:100%;  margin-left: auto; margin-right: auto; transform:translateY(-10px);  left: 0;  right: 0;  box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px; min-height:200px;  border:1px solid rgba(200,200,200,1); background-color:white;  border-radius:15px;">
     

<div class="" style="position:relative; padding:0px 0px 0px 0px; margin:0px;">
 
<div style="padding: 20px; width:100%;">
   <h4 style="font-weight:700; font-size:18px; margin-top:0px;">Resumen de Orden</h4>
        <table style="width:100%; white-space: nowrap; padding:20px; ">
          <tr>
      
            <td style="text-align:left; font-weight:600; font-size:14px; width:65%; color:rgb(102,102,102);  padding:5px 0px 10px 30px;">Examen</td>            
            <td style="text-align:left; font-weight:600; font-size:14px; width:30%; color:rgb(102,102,102);  padding:5px 0px 5px 7px;">Cantidad</td>
            <td style="width:100px; text-align:right; font-weight:600; font-size:14px;  width:20%; color:rgb(102,102,102);  padding:5px 5px;">Precio</td>
          </tr>
          ...
          <tr v-for="e in examenes_agregados">
            ...
            <td style="text-align:left;  overflow: hidden;max-width: 0; text-overflow: ellipsis; font-size:14px; padding:10px 0px; ">
               <div style="position:relative;">
                  <!-- implementar hovering-->
               <b-icon style="color:rgb(7, 136,224); height:12px; width:12px; transform:translateY(-1px); margin-right:12px;" icon="heart-fill"></b-icon>
               {{e.nombre}}
               
               <b-icon style="color:rgb(7, 136,224); height:12px; margin-left:2px; width:12px;" icon="info-circle-fill"></b-icon>
                </div>
               </td>              
            <td style="text-align:left; font-size:14px;">
            <div style="display:inline-block; background:rgb(7, 136,224); line-height:16px; padding:0px 2px; margin:4px; border-radius:4px;">
            <b-icon style="cursor:pointer; color:white; height:13px; width:13px;" v-on:click="cambiarCantidad(e, 'restar')" icon="dash"/> 
            </div>
            <div style="display:inline-block; border:1px solid rgba(102, 102, 102, 0.4); font-size:12px; padding:0px 5px; height:20px; line-height:17px; border-radius:4px;">
               {{e.cantidad}}
            </div>
            <div style="display:inline-block; background:rgb(7, 136,224); line-height:16px; padding:0px 2px; margin:4px; border-radius:4px;">
            <b-icon style="cursor:pointer; color:white; height:13px; width:13px;" icon="plus" v-on:click="cambiarCantidad(e, 'sumar')"/>
            </div>
            </td>

            <td style="text-align:right; font-size:14px;">
               <div style="background:rgb(210, 236, 254); color:rgb(7, 136,224); padding: 2px 8px 2px 0px; border-radius:10px;">
        ...
        <table style=" margin-top:10px;     width: auto;  margin-right: 0px; margin-left: auto; ">

         <tr style="">            
            <td style="text-align:right; font-size:14px;  ">Total exámenes (reembolsable):</td>
            <td style="text-align:right; font-size:14px;">
               ${{Intl.NumberFormat('de-DE').format(examenes_agregados.reduce((x, y)=>{return x+y.precio*y.cantidad}, 0))}}</td>
          </tr>
         <tr style="">            
            <td style="text-align:right; vertical-align:top; font-size:14px; ">Toma de muestra (c/u):</td>
            <td style="text-align:right;">
              <div style="text-decoration:line-through; "> <span v-if="n_personas>1">${{Intl.NumberFormat('de-DE').format((costo_servicio+costo_servicio_pendiente))}}</span></div>  <span style="font-size:14px; color:rgb(108, 193, 252);">${{  Intl.NumberFormat('de-DE').format(Math.round((costo_servicio+(n_personas*costo_servicio_pendiente))/n_personas)) }}</span></td>
          </tr>
        ...
        <button  v-on:click="checkout();" style="color:white; margin-right:0px; margin-top:10px; margin-left:auto; width:200px; padding:7px 0px;" class="agendar-btn" @mouseenter="btnhover = true" @mouseleave="btnhover = false"  :style="{'background': comuna != -1 && examenes_agregados.length > 0 ? 'rgb(254, 177, 53)':'rgb(255,222,166)'}">Ir a agendar <b-icon style="" icon="arrow-right-circle"/></button>
...
-->
  ...
  <div>
   <div class="rego-cont" style="position:relative;  background: linear-gradient(311deg, rgba(2,92,122,1) 0%, rgba(2,134,178,1) 100%); ">
   <!-- :style="{'background-image': 'url(' + require('@/assets/icon/lab.svg') + ')'}"-->
    ...
    <div  class="row featurette">
      ...
      <div class="col-md-5">
         <div style="background:white; width:220px; height:220px; border-radius:120px; border:15px solid rgba(2,92,122,1); vertical-align:middle; margin:0 auto;">
...
<style scoped>
.cajas-sr{
   display:inline-block; background:rgb(7, 136,224); line-height:16px; padding:0px 2px; margin:4px; border-radius:4px;
}
.cf{
   display:grid;
   text-align: center;
...
.h2container{
   margin: 0 auto;
   padding-top:210px;
   margin-top:-0px; background-size: cover; background-repeat: no-repeat;  min-height:750px;
   max-width:800px;
}
.titulo{
