...
<div style="">
    ...
    <div class="main-search">
      ...
      <div style="position:relative;  z-index:-1;"  @mouseenter="hovering_exam = true" @mouseleave="hovering_exam = false" v-on:click="focusInputExam()" :style="{ 'z-index': focused_exam ? '10002':'1', 'width': focused_exam  ? 'Calc(50% - 5px)': focused_comuna ? 'Calc(40% - 5px)' : 'Calc(50% - 5px)', '-webkit-box-shadow': focused_exam ? '2px 3px 25px -1px rgba(0,0,0,0.31)': 'none', 'background':focused_exam ? 'white': ''}" class="main-search-exam dont-close-exam">
...
Examen
      ...
      <div v-if="!focused_exam && examenes_agregados.length >0" style="position:absolute; left:35px; top:23px; z-index:4; color:rgb(102, 102, 102);">
        {{examenes_agregados.reduce((x, y)=>{return x+y.cantidad}, 0)}} {{examenes_agregados.reduce((x, y)=>{return x+y.cantidad}, 0) > 1 ? 'exámenes': 'examen'}}.
      </div>
      <b-icon style="position:absolute; color:rgb(7, 136,224); top:24px; height:20px; width:20px;" icon="heart-fill"></b-icon>
        ...
        <input style="width:70%; font-size:16px; background:none; z-index:6; transform:translate(30px, 13px); " class="dont-close-exam data-hj-allow" v-on:keyup.enter="onEnterExamen()" v-on:input="examen_buscado = $event.target.value" v-model="examen_buscado" :style="{'background':focused_exam ? 'none': ''}" ref="examen"  @focus="focusInputExam(); examen_buscado = ''" type="text" 
        :placeholder="focused_exam ? 'Examen...':''">
...
Comuna 
      ...
      <b-icon style="position:absolute; color:rgb(7, 136,224); top:20px; height:25px; width:25px; z-index:4;" icon="geo-alt-fill"></b-icon>
        ...
        <input style="font-size:16px; width:Calc(100% - 60px); background:none; z-index:6; transform:translate(30px, 12px);" class="dont-close-comuna data-hj-allow" v-on:keyup.enter="onEnterComuna()" v-model="comuna_buscado" :style="{'background':focused_comuna ? 'white': ''}" ref="comuna" @focus="focusInputComuna(); comuna_buscado = ''" type="text"
         :placeholder="focused_comuna ? 'Comuna...':''">
        ...
        :style="{'width': focused_exam || focused_prev || focused_comuna || (examenes_agregados.length > 0 && comuna != -1) ? '50px':'50px',
