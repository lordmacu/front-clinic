...
<div style="">
    ...
    <div id="bexamen" class="main-search" style="margin-bottom:20px;">

      <div style="position:relative;  z-index:-1;"  @mouseenter="hovering_exam = true" @mouseleave="hovering_exam = false" v-on:click="focusInputExam()" :style="{ 'z-index': focused_exam ? '10002':'1', 'width': focused_exam  ? 'Calc(100% - 5px)': focused_comuna ? 'Calc(100% - 5px)' : 'Calc(100% - 5px)', '-webkit-box-shadow': focused_exam ? '2px 3px 25px -1px rgba(0,0,0,0.31)': 'none', 'background':focused_exam ? 'white': ''}" class="main-search-exam dont-close-exam">
...
Examen o prestación
      ...
      <div v-if="!focused_exam && examenes_agregados.length >0" style="position:absolute; left:45px; top:23px; z-index:4; color:rgb(102, 102, 102);">
        {{examenes_agregados.reduce((x, y)=>{return x+y.cantidad}, 0)}} {{examenes_agregados.reduce((x, y)=>{return x+y.cantidad}, 0) > 1 ? 'exámenes seleccionados': 'examen seleccionado'}}.
      </div>
      ...
      <b-icon style="position:absolute; color:rgb(7, 136,224); top:24px; left:20px; height:20px; width:20px;" icon="heart-fill"></b-icon>
        ...
        <input style="width:70%; font-size:16px; background:none; z-index:6; transform:translate(40px, 13px); " class="dont-close-exam data-hj-allow" v-on:keyup.enter="onEnterExamen()" v-on:input="examen_buscado = $event.target.value" v-model="examen_buscado" :style="{'background':focused_exam ? 'none': ''}" ref="examen"  @focus="focusInputExam(); examen_buscado = ''" type="text" 
    ...
    <div v-if="focused_exam"  style=" vertical-align:middle;" class="search-exam-popup dont-close-exam">
      ...
      <div style="padding-left:10px; font-weight:700; font-size:14px; padding-top:0px; color:rgb(7, 136,224); display:inline-block;">Populares: 
      ...
      <!--
      <div v-on:click="agregarExamenAction(examenes.filter(x=>{return x.id == 243})[0])"  style="display:inline-block; cursor:pointer; color:rgb(7, 136,224); font-size:14px; margin:0px 4px; padding: 5px 10px; border-radius:5px; background:rgb(240, 240, 240);">Orina Completa </div>
      <div v-on:click="agregarExamenAction(examenes.filter(x=>{return x.id == 244})[0])"  style="display:inline-block; cursor:pointer; color:rgb(7, 136,224); font-size:14px; margin:0px 4px; padding: 5px 10px; border-radius:5px; background:rgb(240, 240, 240);">Perfil Lipídico </div>
      -->
       <ul style="padding-top:3px;">
         <li class="dont-close-exam " style="height:52px; position:relative; vertical-align:middle; " v-on:click="onClickExam(l); $gtag.event('barrabusqueda-examen', {'event_category': 'busqueda', 'event_label': l.nombre, 'value': 1} );" v-for="l in examenes.slice(0, 6)" >
          <!--
           ...
           -->
            <b-icon style="position:absolute; top: 20px; left:10px; height:14px; width:14px; color:rgb(20, 134, 231);" :icon="examenes_agregados.some(e => e.id == l.id) ? 'check': 'heart-fill'"/> 
           <div class="dont-close-exam" style="font-size: 14px; padding-top:6px; display:inline-block; overflow: hidden; margin-left:30px;  text-overflow: ellipsis; width:Calc(100% - 100px);  white-space: nowrap;">{{l.nombre}}</div>
           <div class="dont-close-exam" style="position:absolute; background:rgb(210, 236, 254); color:rgb(7, 136,224); padding: 2px 8px 2px 6px; border-radius:10px; right:5px; top:14px;">
     ...
     <div class="dont-close-exam" v-else>
      ...
      <div v-on:click="agregarExamenAction(examenes.filter(x=>{return x.id == 242})[0])"  style="display:inline-block; cursor:pointer; color:rgb(7, 136,224); font-size:14px; margin:0px 4px; padding: 5px 10px; border-radius:5px; background:rgb(240, 240, 240);">PCR </div>
      <div v-on:click="agregarExamenAction(examenes.filter(x=>{return x.id == 22})[0])"  style="display:inline-block; cursor:pointer; color:rgb(7, 136,224); font-size:14px; margin:0px 4px; padding: 5px 10px; border-radius:5px; background:rgb(240, 240, 240);">Hemograma </div>
      <!--
      <div v-on:click="agregarExamenAction(examenes.filter(x=>{return x.id == 243})[0])"  style="display:inline-block; cursor:pointer; color:rgb(7, 136,224); font-size:14px; margin:0px 4px; padding: 5px 10px; border-radius:5px; background:rgb(240, 240, 240);">Orina Completa </div>
      <div v-on:click="agregarExamenAction(examenes.filter(x=>{return x.id == 244})[0])"  style="display:inline-block; cursor:pointer; color:rgb(7, 136,224); font-size:14px; margin:0px 4px; padding: 5px 10px; border-radius:5px; background:rgb(240, 240, 240);">Perfil Lipídico </div>
       ...
       <ul style="padding-top:3px;">
         <li class="dont-close-exam " style="height:52px; position:relative; vertical-align:middle; " v-on:click="onClickExam(l.item); $gtag.event('barrabusqueda-examen', {'event_category': 'busqueda', 'event_label': l.item.nombre, 'value': 1} );" v-for="l in resultado_fuse_examen.slice(0, 6)" >
          <!--
           ...
           -->
            <b-icon style="position:absolute; top: 20px; left:10px; height:14px; width:14px; color:rgb(20, 134, 231);" :icon="examenes_agregados.some(e => e.id == l.item.id) ? 'check': 'heart-fill'"/> 
           <div class="dont-close-exam" style="font-size: 14px; padding-top:6px; display:inline-block; overflow: hidden; margin-left:30px;  text-overflow: ellipsis; width:Calc(100% - 100px);  white-space: nowrap;">{{l.item.nombre}}</div>
           <div class="dont-close-exam" style="position:absolute; background:rgb(210, 236, 254); color:rgb(7, 136,224); padding: 2px 8px 2px 6px; border-radius:10px; right:5px; top:14px;">
...
<div class="contenedor-main">
    <div id="bcomuna" class="main-search">

      <div style="position:relative;" @mouseenter="hovering_comuna = true" @mouseleave="hovering_comuna = false" v-on:click="focusInputComuna($event)" :style="{ 'z-index': focused_comuna ? '10002':'1',  'width': focused_comuna  ? '100%': focused_exam ? '100%' : '100%','-webkit-box-shadow': focused_comuna ? '2px 3px 25px -1px rgba(0,0,0,0.31)': 'none', 'background':focused_comuna ? 'white': ''}" class="main-search-comuna">
           <div :class="{'subido2': focused_comuna || comuna != -1, 'bajado2': !focused_comuna && comuna == -1}" 
      
      style=" position:absolute; transition:0.1s; z-index:1; ">
Comuna o ciudad
      </div>
        <div v-if="!focused_comuna && comuna != -1" style="position:absolute; left:50px; top:23px; z-index:4; color:rgb(102, 102, 102);">
        {{comunas_usadas.filter(x=>{return x.id == comuna})[0].Nombre}}.
      </div>
      <b-icon style="position:absolute; color:rgb(7, 136,224); top:20px; left:20px; height:25px; width:25px; z-index:4;" icon="geo-alt-fill"></b-icon>
<b-icon style="position:absolute; right:30px; top:25px; color:rgb(7, 136,224); z-index:4; " :style="{'transform': focused_comuna ? 'rotate(180deg)': 'none'}" icon="chevron-down"></b-icon>   


        <input style="font-size:16px; width:Calc(100% - 60px); background:none; z-index:6; transform:translate(45px, 12px);" class="dont-close-comuna data-hj-allow" v-on:keyup.enter="onEnterComuna()" v-model="comuna_buscado" v-on:input="comuna_buscado = $event.target.value" :style="{'background':focused_comuna ? 'white': ''}" ref="comuna" @focus="focusInputComuna(); comuna_buscado = ''" type="text"
...
<div class="contenedor-main">
    <div style="width:100%; margin-top:15px; position:relative; padding-bottom:10px;">
        ...
        :class="{'glow': comuna != -1 && examenes_agregados.length > 0}"
                 ...
                 <!--'#3D9970':'#FF4136' -->
                 Agendar
                 <b-icon class="dont-close-btn dont-close-exam dont-close-prev dont-close-comuna" style="" icon="calendar"/><div class="dont-close-btn dont-close-exam dont-close-prev dont-close-comuna" :style="{'color': focused_exam || focused_prev || focused_comuna || (examenes_agregados.length > 0) ? 'rgba(255,255,255,1)':'rgba(255,255,255,0)'}" style="margin-left:20px; display:inline; transition-delay:0.3s; font-size:14px; font-weight:700;"></div>
        </button>
    </div>
</div>
    <div class="tags-container">
      ...
      <div v-on:click="agregarExamenAction(examenes.filter(x=>{return x.id == 243})[0])" class="tag">Orina Completa</div>
...
export default {
  ...
  async getMyComuna()
  {
    ...
    focusInputExam() {
      ...
      this.focused_comuna = false;
      this.focused_prev = false;
      var element =  document.getElementById("bexamen")
      var scrollToPos = element.getBoundingClientRect().top + window.scrollY-90
    ...
    focusInputComuna(e) {
      if (typeof e !== 'undefined'){
        ...
        this.focused_prev = false;
      /*
      var element =  document.getElementById("bcomuna")
      var scrollToPos = element.getBoundingClientRect().top + window.scrollY-90
      window.scrollTo(0, scrollToPos)
      */
